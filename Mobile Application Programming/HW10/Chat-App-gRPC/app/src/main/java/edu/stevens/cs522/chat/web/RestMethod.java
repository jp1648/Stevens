...
public class RestMethod {
    ...
    public ChatServiceResponse perform(RegisterRequest request) {
        try {
            ...
            ChatServiceBlockingStub stub = createClient(request.chatServer, request);
    ...
    public UploadObserver perform(SynchronizeRequest request, final DownloadObserver downloadObserver) {
        ...
        ChatServiceStub stub = createStreamingClient(serverUri, request);
        ...
        return new UploadObserver() {
            ...
            public void onSync(long lastSequenceNumber, Double longitude, Double latitude) {
                // TODO
                 SyncRequest syncRequest = SyncRequest.newBuilder().setVersion(lastSequenceNumber)
                 ...
                 UploadItem uploadItem = UploadItem.newBuilder().setRequest(syncRequest).build();
                 requestProducer.onNext(uploadItem);
